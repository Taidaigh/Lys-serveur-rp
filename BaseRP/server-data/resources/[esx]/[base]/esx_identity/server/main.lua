...
AddEventHandler('esx_identity:setIdentity', function(data, myIdentifiers)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			...
			TriggerClientEvent('chatMessage', source, "[IDENTITY]", {255, 0, 0}, "Impossible de définir le caractère, réessayez!!")
		end
	end)
end)
...
TriggerEvent('es:addCommand', 'register', function(source, args, user)
	getCharacters(source, function(data)
		if data.firstname3 ~= '' then
			TriggerClientEvent('chatMessage', source, '[IDENTITY]', {255, 0, 0}, "Vous ne pouvez avoir que 3 caractères enregistrés. Utilisez le ^3/delchar^0  commande afin de supprimer les caractères existants.")
		else
			TriggerClientEvent('esx_identity:showRegisterIdentity', source, {})
		end
	end)
...
TriggerEvent('es:addGroupCommand', 'char', "user", function(source, args, user)
	getIdentity(source, function(data)
		if data.firstname == '' then
			TriggerClientEvent('chatMessage', source, '[IDENTITY]', {255, 0, 0}, "Vous n'avez pas de personnage actif!")
		else
			TriggerClientEvent('chatMessage', source, '[IDENTITY]', {255, 0, 0}, "Caractère actif: ^2" .. data.firstname .. " " .. data.lastname .. "^0")
		end
	end)
end, function(source, args, user)
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Insufficient permissions!")
end, {help = "Liste votre personnage actuel"})
...
TriggerEvent('es:addGroupCommand', 'charlist', "user", function(source, args, user)
	getCharacters(source, function(data)
		if data.firstname1 ~= '' then
			...
			TriggerClientEvent('chatMessage', source, '[IDENTITY]', {255, 0, 0}, "Vous n'avez aucun personnage enregistré Utilisez le ^3/register^0 commande pour enregistrer un personnage.")
		end
	end)
end, function(source, args, user)
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Insufficient permissions!")
end, {help = "Liste tous vos personnages enregistrés"})
...
TriggerEvent('es:addGroupCommand', 'charselect', "user", function(source, args, user)
	...
	if charNumber == nil or charNumber > 3 or charNumber < 1 then
		TriggerClientEvent('chatMessage', source, "[IDENTITY]", {255, 0, 0}, "C'est un caractère invalide!")
		return
	end
		...
		if charNumber == 1 then
			...
			if data.firstname ~= '' then
				updateIdentity(GetPlayerIdentifiers(source)[1], data, function(callback)
					if callback then
						TriggerClientEvent('chatMessage', source, "[IDENTITY]", {255, 0, 0}, "Mise à jour de votre personnage actif ^2" .. data.firstname .. " " .. data.lastname .. "^0!")
					else
						TriggerClientEvent('chatMessage', source, "[IDENTITY]", {255, 0, 0}, "Échec de la mise à jour de votre identité, réessayez!")
					end
				end)
			else
				TriggerClientEvent('chatMessage', source, "[IDENTITY]", {255, 0, 0}, "Vous n'avez pas de personnage dans le slot 1!")
			end
