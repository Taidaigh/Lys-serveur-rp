...
function Message()
    ...
    while messagenotfinish do
		    ...
		    if (GetOnscreenKeyboardResult()) then
		       ...
		       TriggerServerEvent('esx_tabacjob:annonce',result)
...
function OpenVigneActionsMenu()
	...
	if Config.EnablePlayerManagement and PlayerData.job ~= nil and (PlayerData.job.grade_name == 'boss' and PlayerData.job.grade_name ~= 'gerant')then -- Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss'
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 90.0, function(vehicle)
						...
						SetVehicleNumberPlateText(vehicle, "TAB" .. platenum)
		...
		local elements = {
			{label = 'Véhicule de Travail',  value = 'speedo'},
		}
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_tabacjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_tabacjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_tabacjob:hasEnteredMarker', function(zone)
	...
	if zone == 'BriquetFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'tabac'  then
		CurrentAction     = 'briquet_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end

	if zone == 'TraitementTabac1' and PlayerData.job ~= nil and PlayerData.job.name == 'tabac'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone= zone}
	end

	if zone == 'TraitementTabac2' and PlayerData.job ~= nil and PlayerData.job.name == 'tabac'  then
	...
	if zone == 'VigneronActions' and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
		CurrentAction     = 'vigne_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end

	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end

	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
...
AddEventHandler('esx_tabacjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'TabacFarm' or zone =='BriquetFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
		TriggerServerEvent('esx_tabacjob:stopHarvest')
	end
	if (zone == 'TraitementTabac1' or zone == 'TraitementTabac2') and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
		TriggerServerEvent('esx_tabacjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
		TriggerServerEvent('esx_tabacjob:stopSell')
	end
	CurrentAction = nil
end)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'tabac' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_tabacjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_tabacjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					TriggerServerEvent('esx_tabacjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'briquet_harvest' then
					TriggerServerEvent('esx_tabacjob:startHarvest2', CurrentActionData.zone)
				end
				if CurrentAction == 'jus_traitement' then
					TriggerServerEvent('esx_tabacjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'vine_traitement' then
					TriggerServerEvent('esx_tabacjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_tabacjob:startSell', CurrentActionData.zone)
				end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'tabac' and (GetGameTimer() - GUI.Time) > 150 then
...
RegisterNetEvent('esx_tabacjob:annonce')
AddEventHandler('esx_tabacjob:annonce', function(text)
 		texteafiche = text
 		affichenews = true
...
RegisterNetEvent('esx_tabacjob:annoncestop')
AddEventHandler('esx_tabacjob:annoncestop', function()
 		affichenews = false
...
RegisterNetEvent('esx_tabacjob:startSmoke')
AddEventHandler('esx_tabacjob:startSmoke', function(source)
    SmokeAnimation()
end)
