	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			if available or user.getGroup() == "superadmin" then
				...
				TriggerClientEvent('chatMessage', Source, "SYSTEM", {255, 0, 0}, "Vous n'avez pas la permission pour ça")
			end
		end)
	end)
end)
			...
			TriggerEvent('es:canGroupTarget', user.getGroup(), groupsRequired[type], function(available)
				...
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						...
						if type == "kick" then DropPlayer(id, 'Kick par un admin') end
						if type == "ban" then
...
TriggerEvent('es:addGroupCommand', 'ghost', "admin", function(source, args, user)
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Insufficienct permissions!")
end, {help = "Enable or disable ghost"})
...
TriggerEvent('es:addGroupCommand', 'heal', "admin", function(source, args, user)
	if args[2] then
		if(GetPlayerName(tonumber(args[2])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', player, "SYSTEM", {255, 0, 0}, "You have been heal by ^2" .. GetPlayerName(source))
				TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Player ^2" .. GetPlayerName(player) .. "^0 has been heal.")
			end)
		else
			TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Incorrect player ID!")
		end
	...
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Insufficienct permissions!")
end, {help = "Heal a user", params = {{name = "userid", help = "The ID of the player"}}})
...
TriggerEvent('es:addGroupCommand', 'name', "owner", function(source, args, user)
	TriggerClientEvent("es_admin:name", source)
	TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Viewname activé!")
end, function(source, args, user)
    TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Insufficient permissions!")
end)

TriggerEvent('es:addGroupCommand', 'blackout', "owner", function(source, args, user)
	TriggerClientEvent('es:Blackout', -1, toggle)
end)

TriggerEvent('es:addGroupCommand', 'blackoutoff', "owner", function(source, args, user)
	TriggerClientEvent('es:BlackoutOff', -1, toggle)
end)

TriggerEvent('es:addGroupCommand', 'NpcOn', "owner", function(source, args, user)
	TriggerClientEvent('es:NpcOn', -1, toggle)
end)

TriggerEvent('es:addGroupCommand', 'NpcOff', "owner", function(source, args, user)
	TriggerClientEvent('es:NpcOff', -1, toggle)
end)

TriggerEvent('es:addGroupCommand', 'menu', "admin", function(source, args, user)
