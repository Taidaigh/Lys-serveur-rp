function CreateExtendedPlayer(player, accounts, inventory, job, faction, loadout, name, lastPosition)
  ...
  self.setFaction = function(name, grade)
    ...
    MySQL.Async.fetchAll(
      ...
      function(result)
        ...
        self.faction['id']    = result[1].id
        self.faction['name']  = result[1].name
        self.faction['label'] = result[1].label
        ...
        MySQL.Async.fetchAll(
          ...
          function(result)
            self.faction['grade']        = grade
            self.faction['grade_name']   = result[1].name
            self.faction['grade_label']  = result[1].label

            self.faction['skin_male']    = nil
            self.faction['skin_female']  = nil

            if result[1].skin_male ~= nil then
              self.faction['skin_male'] = json.decode(result[1].skin_male)
            end

            if result[1].skin_female ~= nil then
              self.faction['skin_female'] = json.decode(result[1].skin_female)
            end

            TriggerEvent("esx:setFaction", self.source, self.faction, lastFaction)
            TriggerClientEvent("esx:setFaction", self.source, self.faction)
