...
ESX.SavePlayer = function(xPlayer, cb)
  ...
  -- Job, faction, loadout and position
  table.insert(asyncTasks, function(cb)

    MySQL.Async.execute(
      'UPDATE users SET `job` = @job, `job_grade` = @job_grade, `faction` = @faction, `faction_grade` = @faction_grade, `loadout` = @loadout, `position` = @position WHERE identifier = @identifier',
      {
        ['@job']        = xPlayer.job.name,
        ['@job_grade']  = xPlayer.job.grade,
        ['@faction']        = xPlayer.faction.name,
        ['@faction_grade']  = xPlayer.faction.grade,
