AddEventHandler('es:playerLoaded', function(source, _player)
  ...
  TriggerEvent('es:getPlayerFromId', _source, function(player)
    ...
    -- Get faction and loadout
    table.insert(tasks, function(cb)

      local tasks2 = {}

      -- Get faction name, grade
      table.insert(tasks2, function(cb2)
        ...
        MySQL.Async.fetchAll(
          ...
          function(result)
            ...
            userData.faction['name']  = result[1].faction
            userData.faction['grade'] = result[1].faction_grade
      ...
      table.insert(tasks2, function(cb2)
        ...
        MySQL.Async.fetchAll(
          ...
          {
            ['@name'] = userData.faction.name
          },
          function(result)

            userData.faction['label'] = result[1].label
      ...
      -- Get faction grade data
      table.insert(tasks2, function(cb2)

        MySQL.Async.fetchAll(
          'SELECT * FROM `faction_grades` WHERE `faction_name` = @faction_name AND `grade` = @grade',
          {
            ['@faction_name'] = userData.faction.name,
            ['@grade']    = userData.faction.grade
          },
          function(result)

            userData.faction['grade_name']   = result[1].name
            userData.faction['grade_label']  = result[1].label

            userData.faction['skin_male']   = {}
            userData.faction['skin_female'] = {}

            if result[1].skin_male ~= nil then
              userData.faction['skin_male'] = json.decode(result[1].skin_male)
            end

            if result[1].skin_female ~= nil then
              userData.faction['skin_female'] = json.decode(result[1].skin_female)
            end
    ...
    Async.parallel(tasks, function(results)
      ...
      local xPlayer = CreateExtendedPlayer(player, userData.accounts, userData.inventory, userData.job, userData.faction, userData.loadout, userData.playerName, userData.lastPosition)
