...
function SendNotification(options)
    ...
    local options = {
        type = options.type or "infos",
        layout = options.layout or "notyi",
        theme = options.theme or "notyi",
        text = options.text or " ",
        timeout = options.timeout or 5000,
        progressBar = options.progressBar ~= false and true or false,
        closeWith = options.closeWith or {},
        animation = {
        open = options.animation.open or "bounceInUp",
        close = options.animation.close or "bounceOutDown"
        },
        sounds = {
...
RegisterNetEvent("esx_notify:SendNotification")
AddEventHandler("esx_notify:SendNotification", function(options)
    SendNotification(options)
end)

RegisterNetEvent("esx_notify:SetQueueMax")
AddEventHandler("esx_notify:SetQueueMax", function(queue, max)
    SetQueueMax(queue, max)
end)

--[[RegisterNetEvent("chatMessage")
AddEventHandler("chatMessage", function(author, color, text)
    TriggerEvent("esx_notify:SendNotification", {text = "<span style='font-weight: 900'>" .. text .. "</span>",
        layout = "centerLeft",
        timeout = 2000,
        progressBar = false,
        type = "error",
