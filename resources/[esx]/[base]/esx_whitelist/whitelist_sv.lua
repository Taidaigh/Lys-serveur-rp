...
local notwhitelisted = "Vous n'etes pas autorisé à entré sur ce serveur"
...
AddEventHandler('onMySQLReady', function ()
  ...
  AddEventHandler("playerConnecting", function(playerName, reason, deferrals)
	  ...
	  if((onlinePlayers >= PlayersToStartRocade or #PriorityList > 0)  and Vip == false) then
		  ...
		  print("WHITELIST: ANTI SPAM " .. playerName)
		  for i = 1, WaitingTime, 1 do
			  deferrals.update('Connexion dans ' .. tostring(WaitingTime - i) .. ' secondes.')
			  Citizen.Wait(1000)
		  end
		  print("WHITELIST: ANTI SPAM FINI " .. playerName)
  ...
  TriggerEvent('es:addGroupCommand', 'whitelist_load', 'admin', function (source, args, user)
  	
	loadWhiteList()
	TriggerClientEvent('chatMessage', source, "SYSTEM", {0, 255, 0}, "Whitelist : Success whitelist loading")
  end, function (source, args, user)
	TriggerClientEvent('chatMessage', source, 'SYSTEM', { 255, 0, 0 }, 'Tu n\'as pas la permission!')
