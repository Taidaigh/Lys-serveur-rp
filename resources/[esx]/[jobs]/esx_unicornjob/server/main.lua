...
RegisterServerEvent('esx_unicornjob:annonce')
AddEventHandler('esx_unicornjob:annonce', function(result)
	local _source  = source
	local xPlayer  = ESX.GetPlayerFromId(_source)
	local xPlayers = ESX.GetPlayers()
	local text     = result
	...
	for i=1, #xPlayers, 1 do
 		...
 		TriggerClientEvent('esx_unicornjob:annonce', xPlayers[i],text)
	end
	...
	for i=1, #xPlayers, 1 do
 		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
 		TriggerClientEvent('esx_unicornjob:annoncestop', xPlayers[i])
	end
